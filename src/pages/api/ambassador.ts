...
export const POST: APIRoute = async ({ request, locals }) => {
    ...
    const EMAILS = runtime?.env?.EMAIL_RECIPIENTS_AMB || import.meta.env.EMAIL_RECIPIENTS_AMB;
    ...
    const GOOGLE_SHEETS_URL = runtime?.env?.GOOGLE_SHEETS_AMBASSADOR || import.meta.env.GOOGLE_SHEETS_AMBASSADOR;
    ...
    const subjectData = base64File ? `Nueva postulación: Bolsa de Trabajo` : 
        `Nuevo Lead Ambassador: ${data.get('Nombre')} ${data.get('Apellido')}`;
