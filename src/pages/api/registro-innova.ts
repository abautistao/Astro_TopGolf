...
export const POST: APIRoute = async ({ request, locals }) => {
    ...
    const GOOGLE_SHEETS_URL = runtime?.env?.GOOGLE_SHEETS_INNOVA_REGISTRO || import.meta.env.GOOGLE_SHEETS_INNOVA_REGISTRO;
    ...
    const formFields = entries.map(([key, value]) => {
        ...
        if(key === '¿Desea recibir información vía correo electrónico?') {
            fieldValue = value ? "Sí" : "No"
        }
    ...
    const subjectData =  `Nuevo REGISTRO INNOVA: ${data.get('Nombre')}`;
